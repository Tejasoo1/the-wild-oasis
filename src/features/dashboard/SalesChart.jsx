...
/*
1] So, we will want one area for the total sales & one for the extra sales.
   And the label for each of them will be this date.
...
function SalesChart({ bookings, numDays }) {
  ...
  /*
   1] So, what we need to do now is to basically compute a datastructure like the above one.
      (So an array which contains one object for each day)
   2] And that might include even days where they are no sales at all.
      Butt we still will want this chart here to have basically 1 entry for each of the days.
      
   3] So, let's compute this kind of data here from stratch using the bookings array that we received.
 
   4] Here the start date will basically be:-
      today - the no. of days that we want to look at
      For ex:-
      if no. of days = 7 days, then today's date - 7 days 
  ...
  */
  ...
  const data = allDates.map((date) => {
    return {
      label: format(date, "MMM dd"), // "" ---> string which basically encodes the format that we want.
      totalSales: bookings
        .filter(
          (booking) => isSameDay(date, new Date(booking.created_at)) //So this library always needs to work with JS Date objects.
        ) //With this func. we can check whether the 2 dates are the same
  ...
  return (
    <StyledSalesChart>
      ...
      <ResponsiveContainer height={300} width="100%">
        <AreaChart data={data}>
          {/*
            1] And so this is where we specify what data this area should be based on.
            2] In order to make the chart visible what we are missing here, is the height &
               width props.
            3] So, we actually need to give this chart some dimensions.   
         */}
          <XAxis
            dataKey="label"
